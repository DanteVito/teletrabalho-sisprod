...
def manifestacao_interesse_create(request):
    lotacao_servidor = Lotacao.objects.filter(servidor__user=request.user).last()
                ...
                if posto_trabalho_form == lotacao_servidor.posto_trabalho:
                    for m in manifestacoes.filter(aprovado_chefia="aprovado"):
                        if m.lotacao_chefia == lotacao_chefia:
                            messages.info(
                                request,
                                f"Já existe manifestação de interesse aprovada pela chefia: {lotacao_chefia.servidor}.",
                            )
                            return redirect(reverse("webapp:manifestacao_interesse"))
                form.save()
            else:
...
def manifestacao_interesse_delete(request, pk):
    ...
    if instance.aprovado_chefia:
        messages.warning(
            request,
            "Não é possível deletar uma Manifestação de Interesse já aprovada/reprovada!",
        )
    else:
...
def manifestacao_interesse_aprovado_chefia(request, pk):
    ...
    return render(
        request, "webapp/pages/manifestacao-interesse-aprovado-chefia.html", context
    )
...
def declaracao_nao_enquadramento(request):
    ...
    if not last_manifestacao:
        messages.info(
            request,
            "É necessário ter a Manifestação de Interesse cadastrada antes de Preencher a Declaração de Não Enquadramento nas Vedações!",
        )
        return redirect(reverse("webapp:manifestacao_interesse"))
    ...
    context = {
        ...
        "declaracoes_autorizacoes": zip(
            declaracoes_pendentes_gab, autorizacoes_excecoes
        ),
    }
...
def declaracao_nao_enquadramento_create(request):
    ...
    if request.method == "POST":
        form = DeclaracaoNaoEnquadramentoVedacoesForm(request.POST, user=request.user)
        if form.is_valid():
            obj = form.save(commit=False)
            ...
            if not manifestacao in ManifestacaoInteresse.objects.filter(
                lotacao_servidor__servidor__user=request.user
            ):
                return HttpResponseBadRequest("not allowed")

            if DeclaracaoNaoEnquadramentoVedacoes.objects.filter(
                manifestacao=manifestacao
            ):
                messages.info(
                    request,
                    "Não é possível cadastrar mais de uma Declaração para mesma Manifestação de Interesse!",
                )
                return redirect(reverse("webapp:declaracao_nao_enquadramento"))

            obj.adicionado_por = request.user
            obj.modificado_por = request.user
            modelo_aprovacao_excecao = ModeloDocumento.objects.get(
            ...
            if not obj.cargo_chefia_direcao or lotacao_servidor.posto_trabalho.chefia:
                if not obj.justificativa_excecao:
                    messages.info(
                        request,
                        "É necessário preencher a justificativa para exceção de teletrabalho das chefias!",
                    )
                    return redirect(
                        reverse("webapp:declaracao_nao_enquadramento_create")
                    )
                obj.save()
    ...
    context = {"tipo_form": "create", "form": form}
    return render(
...
def plano_trabalho(request):
    ...
    if not DeclaracaoNaoEnquadramentoVedacoes.objects.filter(
        ...
        messages.info(
            request,
            "É necessário cadastrar a Declaração de Não Enquadramento nas Vedações Legais antes de cadastrar o Plano de Trabalho!",
        )
        return redirect(reverse("webapp:declaracao_nao_enquadramento"))
...
def plano_trabalho_create(request):
    ...
    if not last_manifestacao.aprovado_chefia:
        messages.info(
            request,
            "É necessário ter a Manifestação de Interesse aprovada antes de cadastrar o Plano de Trabalho!",
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            periodos_formset = PeriodoTeletrabalhoFormSet(request.POST, instance=obj)
...
def plano_trabalho_edit(request, pk):
    ...
    if request.method == "POST":
        form = PlanoTrabalhoForm(
            request.POST,
            instance=instance,
            user=instance.manifestacao.lotacao_servidor.servidor.user,
        )
        if form.is_valid():
            obj = form.save(commit=False)
            obj.modificado_por = request.user
...
def plano_trabalho_aprovado_chefia(request, pk):
    ...
    form_instance = PlanoTrabalhoForm(
        instance=plano_trabalho,
        user=plano_trabalho.manifestacao.lotacao_servidor.servidor.user,
    )
...
def plano_trabalho_aprovado_cigt(request, pk):
    ...
    if request.user.groups.filter(name="CIGT"):
        plano_trabalho = PlanoTrabalho.objects.get(pk=pk)
        form_instance = PlanoTrabalhoForm(instance=plano_trabalho, user=request.user)
            ...
            if form.is_valid():
                ...
                if obj.aprovado_cigt == "aprovado":
                    ...
                    if not DespachoCIGTPlanoTrabalho.objects.filter(plano_trabalho=obj):
                        modelo_plano_trabalho = ModeloDocumento.objects.get(
                            nome_modelo="PARECER PLANO DE TRABALHO CIGT"
                        )  # noqa E501
                        modelo_protocolo_autorizacao = ModeloDocumento.objects.get(
                            nome_modelo="PROTOCOLO AUTORIZACAO TELETRABALHO"
                        )  # noqa E501

                        # cria DespachoCIGTPlanoTrabalho
                        despacho_cigt_plano_trabalho = (
                            DespachoCIGTPlanoTrabalho.objects.create(
                                plano_trabalho=obj,
        ...
        return render(
            request, "webapp/pages/plano-trabalho-aprovado-cigt.html", context
        )
    return HttpResponseBadRequest("proibido")
...
def periodo_teletrabalho(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                reverse(
                    "webapp:plano_trabalho_edit", kwargs={"pk": obj.plano_trabalho_id}
                )
            )
...
def periodo_teletrabalho_delete(request, pk):
    ...
    if instance.plano_trabalho.aprovado_chefia:
        messages.error(
            request,
            "Não é possível excluir um Período de um Plano de Trabalho já aprovado/reprovado!",
        )
    else:
        instance.delete()
    return redirect(
        reverse("webapp:plano_trabalho_edit", kwargs={"pk": instance.plano_trabalho_id})
    )
...
def atividade_teletrabalho(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                reverse("webapp:plano_trabalho_edit", kwargs={"pk": plano_trabalho_id})
            )
...
def atividade_teletrabalho_delete(request, pk):
    ...
    if instance.periodo.plano_trabalho.aprovado_chefia:
        messages.error(
            request,
            "Não é possível excluir uma Atividade de um Plano de Trabalho já aprovado/reprovado!",
        )
    else:
        instance.delete()
    return redirect(
...
def atividade_cumprimento(request, pk_avaliacao, pk_atividade, cumprimento):
    ...
    return redirect(
        reverse("webapp:avaliacao_atividades_list", kwargs={"pk": pk_avaliacao})
    )
...
def autorizacao_excecao_edit(request, pk):
    ...
    if request.user.groups.filter(name="GABINETE"):
        ...
        form_instance = AutorizacoesExcecoesForm(instance=autorizacao)
        form = AutorizacoesExcecoesAprovaForm(instance=autorizacao)

        if request.method == "POST":
            form = AutorizacoesExcecoesAprovaForm(request.POST, instance=autorizacao)
...
def cigt(request):
    ...
    if request.user.groups.filter(name="CIGT"):
        ...
        protocolos_autorizacao_teletrabalho = (
            ProtocoloAutorizacaoTeletrabalho.objects.all()
        )
        context = {
...
def cigt_analisar_protocolos_autorizacao(request):
    ...
    return render(
        request, "webapp/pages/cigt-analisar-protocolos-autorizacao.html", context
    )
...
def cigt_analisar_avaliacoes_mensais(request):
    ...
    return render(
        request, "webapp/pages/cigt-analisar-avaliacoes-mensais.html", context
    )
...
def cigt_controle_mensal_teletrabalho(request):
    ...
    return render(
        request, "webapp/pages/cigt-controle-mensal-teletrabalho.html", context
    )
...
def verificar_retorno_avaliacoes_cigt(request):
    ...
    if request.user.groups.filter(name="CIGT"):
        ...
        return render(
            request, "webapp/pages/verificar-retorno-avaliacoes-cigt.html", context
        )
    return HttpResponseBadRequest("proibido-cigt")
...
def protocolos_autorizacao_teletrabalho(request):
    ...
    return render(
        request, "webapp/pages/protocolos-autorizacao-teletrabalho.html", context
    )
...
def protocolo_autorizacao_teletrabalho_edit(request, pk):
    ...
    return render(
        request, "webapp/pages/protocolo-autorizacao-teletrabalho-form.html", context
    )
...
def chefia_imediata_analisar_manifestacoes(request):
    ...
    return render(
        request, "webapp/pages/chefia-imediata-analisar-manifestacoes.html", context
    )
...
def chefia_imediata_analisar_planos_trabalho(request):
    ...
    return render(
        request, "webapp/pages/chefia-imediata-analisar-planos-trabalho.html", context
    )
...
def chefia_imediata_realizar_avaliacoes_mensais(request):
    if request.user.groups.filter(name="CHEFIAS"):
        ...
        return render(
            request, "webapp/pages/chefia-imediata-avaliacoes-mensais.html", context
        )
    return HttpResponseBadRequest("proibido-chefia-imediata")
...
def avaliacao_atividades_list(request, pk):
    ...
    if request.user.groups.filter(name="CHEFIAS") or request.user.groups.filter(
        name="CIGT"
    ):
        ...
        if not avaliacoes_chefia.filter(pk=pk) and not request.user.groups.filter(
            name="CIGT"
        ):
        ...
        if request.user.groups.filter(name="CHEFIAS"):
            if request.method == "POST":
                form = AvaliacaoChefiaFinalizaForm(request.POST, instance=avaliacao)

                if form.is_valid():
                    for atividade, cumprimento in zip(
                        atividades_para_avaliacao, request.POST.getlist("cumprimento")
                    ):
                        if atividade.cumprimento:
                            if atividade.cumprimento != cumprimento:
                                messages.info(
                                    request,
                                    f"É necessário justificativa para alterar: {atividade.cumprimento} -> {cumprimento}",
                                )
                                request.session["atividade_pk"] = atividade.pk
                                request.session["novo_cumprimento"] = cumprimento
                                return redirect(
                                    reverse(
                                        "webapp:chefia_imediata_alterar_avaliacao_mensal",
                    ...
                    form.save()
                    return redirect(
                        reverse("webapp:chefia_imediata_realizar_avaliacoes_mensais")
                    )
...
def chefia_imediata_alterar_avaliacao_mensal(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            obj.save()
            return redirect(
                reverse("webapp:avaliacao_atividades_list", kwargs={"pk": pk})
            )
    ...
    return render(
        request, "webapp/pages/chefia-imediata-alterar-avaliacao-form.html", context
    )
...
def avaliacao_chefia_atividade(request, pk):
    ...
    formset = AtividadesTeletrabalhoFormSet(instance=periodo)
...
def gerar_portaria_doe(request):
    if request.user.groups.filter(name="CIGT"):
        ...
        if not obj:
            # posteriormente separara as duas situações:
            # 1) em que falta o sid
            # 2) em que não há modificação para publicar
            messages.error(
                request,
                "VERIFIQUE TODOS OS SIDS NOS PROTOCOLOS DE AUTORIZACAO | NÃO HÁ NENHUMA MODIFICAÇÃO",
            )
            return redirect(reverse("webapp:cigt_portarias_doe"))
        obj.render_docx_tpl(tipo_doc="portaria_teletrabalho_doe")
        try:
            if os.path.exists(obj.docx.path):
                fh = open(obj.docx.path, "rb")
                fh.close()
                return redirect(reverse("webapp:cigt_portarias_doe"))
        except ValueError:
            raise Exception(
            ...
            if os.path.exists(obj.docx.path):
                ...
                with open(obj.get_path_file_temp(tipo_doc="docx"), "rb") as fh:
                    ...
                    response["Content-Disposition"] = (
                        f'filename={obj.gerar_nome_arquivo("portaria_teletrabalho_doe")}'  # noqa E501
                    )
                    return response
        except ValueError:
            raise Exception(
    ...
    if True:  # request.user.groups.filter(name='CIGT'):
        if model == "portaria":
            ...
            with open(obj.volume_docx.path, "rb") as fh:
                ...
                response["Content-Disposition"] = (
                    f"filename={obj.gerar_nome_arquivo(nome_arquivo)}"  # noqa E501
                )
                return response
        else:
...
def htmx_adiciona_periodo_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        for data_inicio in periodos_salvos:
            last_day_month = calendar.monthrange(data_inicio.year, data_inicio.month)[1]
            data_fim = date(data_inicio.year, data_inicio.month, last_day_month)
            ...
            atividades_teletrabalho_first_periodo = (
                AtividadesTeletrabalho.objects.filter(periodo=first_periodo)
            )
...
def htmx_adiciona_atividade_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        for periodo in periodos:
            atividade = ListaAtividades.objects.get(id=request.POST.get("atividade"))

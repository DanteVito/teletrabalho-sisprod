...
def create_unidades(file: str) -> None:
    """
    Função que migra os dados relativos às
    ...
    for unidade in unidades:
        data_unidade = {"nome": unidade, "sigla": unidade}
        obj = Unidade.objects.update_or_create(**data_unidade)
        print(f"[+] Criada unidade: {obj}")


def create_setores_postos_trabalho(file: str) -> None:
    """
    Criar setores e postos de trabalho
    """
    with open(file, "r", encoding="utf-8") as f:
        for line in f:
            data_line = line.split("\t")
            sigla = data_line[0].split("/")[1]
            unidade = Unidade.objects.get(sigla=sigla)  # pylint: disable=E1101
            setor = data_line[0].split("/")[2]
            data_setor = {"unidade": unidade, "nome": setor, "sigla": setor}
            setor, _ = Setor.objects.update_or_create(**data_setor)

            posto_trabalho = data_line[1].strip()
            tipo_posto_trabalho = data_line[2].strip()
            chefia_data = data_line[3].strip()

            if chefia_data == "CHEFIA":
                chefia = True
            else:
                chefia = False
            ...
            posto_trabalho, posto_trabalho_created = (
    ...
    with open(file, "r", encoding="utf-8") as f:
        for item in f:
            ...
            try:
                ...
                print(f"[!] {user.username} - {user.nome} -> já existente")
            ...
            try:
                ...
                setor = Setor.objects.get(unidade=unidade, sigla=setor_sigla)
                posto_trabalho = PostosTrabalho.objects.get(
                    setor=setor, posto=posto_trabalho_posto
                )

                lotacao, created = Lotacao.objects.update_or_create(
                    servidor=servidor, posto_trabalho=posto_trabalho
                )

                if not created:
                    print(f"[+ posto de trabalho {posto_trabalho} -> {servidor}]")
    ...
    with open(file, "r", encoding="utf-8") as f:
        for item in f:
            ...
            posto_trabalho_chefia = PostosTrabalho.objects.get(
                setor=setor_chefia, posto=posto_posto_trabalho_chefia
            )
...
def migrate(file: str, model, field: str) -> None:
    """
    Função genérica para migração de dados
    """
    with open(file, "r", encoding="utf-8") as f:
        for value in f:
